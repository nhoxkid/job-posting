	...
	return (
		<div style={{ animation: 'spr-up .35s ease both' }}>
			...
			<div style={{ maxWidth: 1000, margin: '0 auto', padding: '36px 28px 60px' }}>
				<h1 style={{ fontFamily: "'Schibsted Grotesk'", fontWeight: 800, fontSize: 30, letterSpacing: '-0.02em', margin: '0 0 6px' }}>Frequently Asked Questions</h1>
				<p style={{ fontSize: 16, color: '#566660', margin: '0 0 32px' }}>Helpful suggestions and general advice on applications and the job board.</p>
				<div style={{ display: 'flex', flexDirection: 'column', gap: 34 }}>
					{faqRaw.map((g, gi) => (
						<div key={g.title}>
							<h2 style={{ fontFamily: "'Schibsted Grotesk'", fontWeight: 700, fontSize: 18, letterSpacing: '-0.01em', margin: '0 0 14px', color: '#15603F' }}>{g.title}</h2>
							<div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
								{g.items.map((it, ii) => {
									const id = `g${gi}-${ii}`
									const isOpen = openFaq === id
									return (
										<div key={id} onClick={() => setOpenFaq(isOpen ? null : id)} className='v1-faq-card' style={{ background: '#fff', border: '1px solid #E8EDEB', borderRadius: 14, padding: '18px 20px', cursor: 'pointer', transition: 'border-color .15s' }}>
											<div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
												<span style={{ fontFamily: "'Schibsted Grotesk'", fontWeight: 700, fontSize: 15.5, lineHeight: 1.35 }}>{it.q}</span>
												<span style={{ color: '#1A7A52', fontSize: 18, fontWeight: 700, flexShrink: 0, lineHeight: 1.2 }}>{isOpen ? '−' : '+'}</span>
											</div>
											{isOpen && <p style={{ fontSize: 14, lineHeight: 1.6, color: '#566660', margin: '12px 0 0' }}>{it.a}</p>}
										</div>
									)
								})}
							</div>
