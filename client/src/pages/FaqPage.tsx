...
const faqGroups = [
  ...
  },
  {
    ...
    items: [
      ...
      { q: 'What does "posted 30 days ago" usually mean?', a: 'Older postings are often still open but draw far more applicants. We surface the posting date so you can prioritise fresh, less-competitive roles.' },
...
export function FaqPage() {
  ...
  return (
    <div style={{ maxWidth: 1000, margin: '0 auto', padding: '36px 28px 60px' }}>
      <h1 className="rv-display" style={{ fontWeight: 800, fontSize: 32, letterSpacing: '-0.025em', margin: '0 0 6px' }}>Frequently Asked Questions</h1>
      <p style={{ fontSize: 16, color: '#5C6B63', margin: '0 0 32px' }}>Helpful suggestions and general advice on applications and the job board.</p>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 34 }}>
        {faqGroups.map((g, gi) => (
          <div key={g.title}>
            <h2 className="rv-display" style={{ fontWeight: 700, fontSize: 18, letterSpacing: '-0.01em', margin: '0 0 14px', color: '#12805A' }}>{g.title}</h2>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              {g.items.map((it, ii) => {
                const id = `g${gi}-${ii}`
                const isOpen = open === id
                return (
                  <div key={id} onClick={() => setOpen(isOpen ? null : id)} className="rv-faq-card" style={{ background: '#fff', border: '1px solid #E6ECE7', borderRadius: 16, padding: '18px 20px', cursor: 'pointer', boxShadow: '0 1px 2px rgba(10,20,16,0.04)', transition: 'border-color .15s', alignSelf: 'start' }}>
                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
                      <span className="rv-display" style={{ fontWeight: 700, fontSize: 15.5, lineHeight: 1.35 }}>{it.q}</span>
                      <span style={{ color: '#12805A', fontSize: 18, fontWeight: 700, flexShrink: 0, lineHeight: 1.2 }}>{isOpen ? '−' : '+'}</span>
                    </div>
                    {isOpen && <p style={{ fontSize: 14, lineHeight: 1.6, color: '#5C6B63', margin: '12px 0 0' }}>{it.a}</p>}
                  </div>
                )
              })}
            </div>
